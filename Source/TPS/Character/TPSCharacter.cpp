		...
		if (myController)
		{
			...
			if (CurrentWeapon)
			{
				...
				case EMovementState::Run_State:
					...
					CurrentWeapon->ShouldReduceDispersion = false;
					break;
				case EMovementState::CrouchWalk_State:
					Displacement = FVector(0.0f, 0.0f, 180.0f);
					CurrentWeapon->ShouldReduceDispersion = false;
					break;
...
void ATPSCharacter::CharacterUpdate()
{
	...
	case EMovementState::Run_State:
		...
		break;
	case EMovementState::CrouchWalk_State:
		ResSpeed = MovementSpeedInfo.CrouchWalk;
		break;
	case EMovementState::SprintRun_State:
		...
		else
		{
			...
			else
			{
				...
				else
				{
					if (!WalkEnabled && !SprintRunEnabled && AimEnabled)
					{
						MovementState = EMovementState::CrouchWalk_State;
					}
				}
			}
		}
